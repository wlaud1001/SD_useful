...
def inverseSD(
    ...
    if from_pretrained is None:
        ...
        unet = unet.to(device)
        vae = vae.to(device)
        clip_text_encoder = text_encoder.to(device)
        clip_tokenizer = tokenizer.to(device)
    ...
    with autocast(device):
        ...
        tokens_unconditional = clip_tokenizer(uncond_prompt, padding="max_length", max_length=self.clip_tokenizer.model_max_length, truncation=True, return_tensors="pt", return_overflowing_tokens=True)
        embedding_unconditional = clip_text_encoder(tokens_unconditional.input_ids.to(device)).last_hidden_state

        tokens_conditional = clip_tokenizer(prompt, padding="max_length", max_length=self.clip_tokenizer.model_max_length, truncation=True, return_tensors="pt", return_overflowing_tokens=True)
